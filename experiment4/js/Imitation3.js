...
void main() {
  ...
  for(int i = 0; i < 50; i++){
    ...
    float LDist2 = 1.0 - (dist2*2.0);
    count += 1.0;
    ColorSumR += ColorArrayR[i]*IDist2;
    ColorSumG += ColorArrayG[i]*IDist2;
    ColorSumB += ColorArrayB[i]*IDist2;
    sum = sum + IDist2;
  }
  float fval = 0.0;
  //if(IDist + IDist2 > 0.7){
  ...
  vec4 finalColor = vec4(10.0*ColorSumR/count, 10.0*ColorSumG/count, 10.0*ColorSumB/count, 1.0);
