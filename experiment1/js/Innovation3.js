...
function setup() {
  ...
  for(var i = 0; i < grid.length; i++){
    for(var o = 0; o < h; o++){
      ...
      grid[i].push({x:(i + random(-rand, rand))*10 + 10, y:(o + random(-rand, rand))*10 + 10, xs:0, ys:0, Links:[], color:{r:random(0,255), b:random(0,255), g:random(0,255)}});
    }
  }
  for(var i = 0; i < grid.length; i++){
...
function draw() {
  ...
  for(var i = 0; i < grid.length; i++){
    for(var o = 0; o < grid[i].length; o++){
      ...
      ellipse(grid[i][o].x +grid[i][o].xs + cos(grid[i][o].y + t/9 + 1241)*5 - 20, grid[i][o].y + grid[i][o].ys+sin(t/10 + grid[i][o].x)*5 - 20, 3, 3);
    }
  }
  if(mouseIsPressed && T < 0){
    ripples.push({x:mouseX/(800/500) + 20, y:mouseY/(800/500) + 20, r:0, d:false});
    T = 30;
  }
